...
char** fuzz_nalloc_backtrace_strs = NULL;
...
static void fuzz_nalloc_sig_handler(int signum, siginfo_t *siginfo, void *context) {
    ...
    printf("NULL alloc in %d run:\n", fuzz_nalloc_runs);
    for (int i = 0; i < fuzz_nalloc_backtrace_frames; ++i) {
        printf("%s\n", fuzz_nalloc_backtrace_strs[i]);
    }
...
static void fuzz_nalloc_save_backtrace(void) {
    void* callstack[128];
    fuzz_nalloc_backtrace_frames = backtrace(callstack, 128);
    if (fuzz_nalloc_backtrace_strs != NULL) {
        free(fuzz_nalloc_backtrace_strs);
    }
...
static bool fuzz_nalloc_fail(size_t size) {
    ...
    //TODO parameter for bitmask how often
    if (((fuzz_nalloc_random_state ^ 0x294cee63) & fuzz_nalloc_bitmask) == 0 && fuzz_nalloc_runs != 1) {
        void* callstack[2];
        int frames = backtrace(callstack, 2);
        char** strs = backtrace_symbols(callstack, frames);
        if (frames == 2) {
            if (fuzz_nalloc_exclude != NULL && strstr(strs[1], fuzz_nalloc_exclude) != NULL) {
                return false;
            }
            // backtrace_symbols calls malloc !
            // TODO have stack trace without alloc
            if (strstr(strs[0], "fuzz_nalloc") != NULL) {
                return false;
            }
        }
...
void fuzz_nalloc_init() {
    ...
    fuzz_nalloc_exclude =  getenv("NALLOC_FUZZ_EXCLUDE");
    if (fuzz_nalloc_exclude == NULL) {
        fuzz_nalloc_exclude = "_ZN5alloc";
    }
